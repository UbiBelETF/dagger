...
{
    template<typename T>
    Sequence<T>::Sequence(UInt32 repeats_, ::Sequence<T> choices_)
            : m_Choices{ m_Choices }
            , m_Generator{}
            , m_Repeats{ repeats_ }
        {
    ...
    template<typename T>
    Sequence<T>::Sequence(UInt32 repeats_, std::initializer_list<T>&& choices_)
    ...
    template<typename T>
    Sequence<T>::Sequence(::Sequence<T> choices_)
            : m_Choices{ m_Choices }
            , m_Generator{}
            , m_Repeats{ 2 }
        {
    ...
    template<typename T>
    Sequence<T>::Sequence(std::initializer_list<T>&& choices_)
            : m_Choices{ std::move(choices_) }
            , m_Generator{}
            , m_Repeats{ 2 }
        {
    ...
    template<typename T>
    Sequence<T>::Sequence(const Sequence<T>& other_)
    ...
    template<typename T>
    Sequence<T>::Sequence(const Sequence<T>& other_, ::Sequence<T> pregen_)
            : m_Choices{ other_.m_Choices }
            , m_Generator{ pregen_ }
            , m_Repeats{ other_.m_Repeats }
        {
    ...
    template<typename T>
    ::Sequence<T> Sequence<T>::GetPool() const
    {
        return m_Generator;
    }
