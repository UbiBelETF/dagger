...
Entity Empty(Registry& reg_, UInt32 x_, UInt32 y_)
    ...
    Entity CreateBlankWall(Registry& reg_, UInt32 x_, UInt32 y_)
    {
        Entity entity = reg_.create();
        auto& sprite = reg_.emplace<Sprite>(entity);
        AssignSprite(sprite, "spritesheets:lab:wall_bottom_5_blank");
        sprite.position = { x_ * 16-125.0f, y_ * 16-100.0f, 30 };
        return entity;
    }
    ...
    {   
        ...
        sprite.position = { x_ * 16-125.0f, y_ * 16-100.0f, 30 };
        auto& idd=reg_.emplace<CollideW>(entity);
        idd.id=0;
    ...
    {   
        ...
        sprite.position = { x_ * 16-125.0f, y_ * 16-100.0f, 30 };
        //Doesnt go to the right position on the map if room changes 
        auto& trn=reg_.emplace<Transform>(entity);
        trn.position=sprite.position;
        auto& idd=reg_.emplace<CollideW>(entity);
        idd.id=0;
